...
router.post('/', async (req, res) => {
  ...
  const { rows } = await pool.query(
    `insert into contacts (name,email,phone,company,source,status,tags,owner_id) values ($1,$2,$3,$4,$5,$6,$7,$8)
     returning *`, [name, email || null, phone || null, company || null, source, status, tags, req.user.id]
  );
